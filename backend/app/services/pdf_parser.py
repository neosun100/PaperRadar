...
class PDFParser:
    ...
    def _merge_text_blocks(self, blocks: List[dict]) -> List[dict]:
        ...
        for next_block in sorted_blocks[1:]:
            ...
            should_merge = (
                v_gap >= -5.0 and v_gap < 15.0 and  # 垂直间距更严格 (was 20)
                h_align_diff < 15.0 and  # 水平对齐更严格 (was 20)
                current["rotation"] == next_block["rotation"] and
                combined_len < 500  # 合并后文本不超过500字符
            )
